...
//		this.prev = (this.begin == 1) ? false : true;
		this.prev = begin > 1;
		this.next = (articlceTotalCount <= (end * page.getAmount()) ) ? false : true;
		
		if(!this.next) {
			this.end = (int) Math.ceil(articlceTotalCount/ (double) page.getAmount());
		}
