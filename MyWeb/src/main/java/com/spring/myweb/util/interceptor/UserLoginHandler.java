...
public class UserLoginHandler implements HandlerInterceptor {
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		if(request.getMethod().equals("POST")) { // 요청 방식이 POST 라면 -> 로그인 요청이었다면
			ModelMap map = modelAndView.getModelMap(); // 모델 객체 꺼내기
			String result = (String) map.get("result"); //get id info or null from model named 'result'
			...
			if(result != null) { //로그인 성공
				...
				//로그인 성공한 사람은 여기서 바로 메인 화면으로 이동햠.
			} else {
				//로그인 실패
				modelAndView.addObject("msg", "loginFail"); //msg라는 이름으로 'loginFail'이라는 문자열을 넣어둘거야
			}
		}
