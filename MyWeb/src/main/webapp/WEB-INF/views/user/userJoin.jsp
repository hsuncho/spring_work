    ...
    <script>
        ...
        document.getElementById('idCheckBtn').onclick = function () {
            ...
            */
            ...
            //fetch('url', {요청 관련 정보를 담은 객체(GET 방식에서는 따로 전달 안 함.)})
            fetch('${pageContext.request.contextPath}/user/\${userId}')
            // fetch('${pageContext.request.contextPath}/user/' + userId)

            //Promise 객체의 상태가 요청 성공일 시 데이터 후속 처리 진행.
            .then(res => { //화살표 함수 내의 코드가 한 줄이고, 그것이 return이라면 괄호와 return 생략 가능
            ...
            //통신이 성공했다는 것을 먼저 보장받은 후, 서버의 데이터를 꺼내는 콜백함수를 시행.
            .then(data => {
        ...
        document.querySelector('.mail-check-input').onblur = function(e) {
            ...
            if(inputCode === code) {
                ...
                //option 변경 시도를 할 때마다 초기값으로 강제 변경해서
        ...
        //다음 주소 api 사용해보기 (script src 추가해야합니다.)
...
document.getElementById('joinBtn').onclick = function() {
    if(idFlag && pwFlag) {
        ...
        if(document.getElementById('mail-check-btn').getAttribute('disabled')) {
