<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <script>
        window.onload = function() { //페이지 로딩 끝나면 실행 시킬게
            ...
            document.getElementById('replyRegist').onclick = () => {
                ...
                const reqObj = {
                    ...
                    body: JSON.stringify({ //json 형태로 이 객체를 문자화할게
                        'bno': bno,
                        'replyText': reply,
                        'replyId': replyId,
                        'replyPw': replyPw
            ...
            document.getElementById('moreList').onclick = () => {
                ...
                getList(++page, false); //먼저 page값 올리고 보내라
            }
            ...
            function getList(pageNum, reset) {
                ...
                .then(res => res.json()) //map이 json으로 변환되어 옴
                .then(data => {
                    ...
                    //응답 데이터의 길이가 0과 같거나 더 작으명 함수를 종료
            ...
            그렇다 보니 이벤트가 등록되는 시점보다 fetch 함수의 실행이 먼저 끝날 것이라는
            보장이 없기 때문에 해당 방식은 이벤트 등록이 불가능합니다.
            이때는 이미 실제로 존재하는 #replyList에 이벤트를 등록하고, 이벤트를 자식에세 위임하여
            ...
            */
            ...
            document.getElementById('replyList').addEventListener('click', e => {
                ...
                //댓글이 여러 개 -> 수정, 삭제가 발생하는 댓글이 몇 번인지도 확인해야한다.
            ...
            //수정 처리 함수. (수정 모달을 열어서 수정 내용을 작성한 후 버튼을 클릭했을 때)
            document.getElementById('modalModBtn').onclick = () => {
                ...
                .then(data => {
                    if(data === 'pwFail') {
                        ...
                        getList(1, true); //기존에 있는 거 다 밀고 새롭게 보여줘야지
                    }
                })
            ...
            if(replyPw === ''){
                ...
                .then(data => {
                    if(data === 'delSuccess') {
                        ...
                        document.getElementById('modalPw').value = '';
                        $('#replyModal').modal('hide');
                        getList(1, true); //기존의 것 밀고 새롭게 보여줘
                    } else {
                        alert('비밀번호가 틀렸습니다!');
                        document.getElementById('modalPw').value = '';
                        document.getElementById('modalPw').focus();
