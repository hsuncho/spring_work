...
<script>
	...
	function regist() {
		...
		if(userId === '') { //세션 데이터 없다? -> 로그인을 안 한 것
		...
		*/
		...
		//file input 내용을 비워주고, 글 영역 비워주고, 
		//class 이름이 fileDiv(미리보기) 영역을 감춰 주세요. -> disply 속성을 none으로
		.then(res => res.text())
		.then(data => {
			console.log(data);
	...
	//이미지 다운로드, 이미지 버튼을 누르면 사진이 클릭이 될 수 있어
	//클릭하면 모달을 띄워줄 거야
	//이벤트 발생 시킬 공간은 이미지 다운로드 링크와 이미지 영역와 댓글 달기
	$contentDiv.addEventListener('click', e => {
		...
		if(e.target.matches('.title #download')) {
			if(confirm('다운로드를 진행할게요.')) {
				...
				//href에 글 번호를 넣었는데 타겟이 되는 태그의 href 속성을 가져오기
			} return;
		}
