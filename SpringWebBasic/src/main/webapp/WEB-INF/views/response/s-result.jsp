...
<body>
	...
	<p>
		...
		<c:choose>
			...
			<c:when test="${login eq 'f-id'}">
				<h2>잘못된 아이디입니다.</h2>
			</c:when>
			<c:when test="${login eq 'f-pw'}">
				<h2>잘못된 비밀번호입니다.</h2>
			</c:when>
		</c:choose>
	</p>
	...
	<%--
		<h1>
			<c:if test="${result == 'success'}">로그인에 성공하셨습니다.</c:if>
	...
	<script>
		/*
			...
			-> 응답은 HTML로 표현
			EL 표현식이 서버에서 먼저 평가되고 그 결과를 클라이언트에게 전달하여
			브라우저로 표현하기 때문에 script에서도 EL 표현이 가능합니다.
